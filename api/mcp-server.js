...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    return res.json({
      name: 'DealMate MCP Server',
      version: '1.0.0',
      services: server.services.size,
      status: 'running'
    });
  }
  ...
  if (req.method === 'POST') {
    try {
      ...
      switch (message.method) {
        case 'resources/list':
          response = await server.handleListResources();
          break;
        case 'resources/read':
          response = await server.handleReadResource(message.params.uri);
          break;
        default:
